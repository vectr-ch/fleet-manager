...
function AcceptInviteContent() {
  ...
  const acceptInviteMutation = trpc.sysadminResetPassword.acceptInvite.useMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await acceptInviteMutation.mutateAsync({ token: token!, password });
      setSuccess(true);
      setTimeout(() => {
    ...
    <div className="flex min-h-screen items-center justify-center bg-black">
      <div className="w-full max-w-md space-y-6 rounded-lg border border-neutral-800 bg-neutral-900 p-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-neutral-500">Set your password to activate your account</p>
        </div>
        ...
        {!token ? (
          ...
          <div className="space-y-4 text-center">
            ...
            <p className="text-lg font-medium text-white">Account activated</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? "Setting password..." : "Set password and activate"}
            </button>
          </form>
        )}
      </div>
